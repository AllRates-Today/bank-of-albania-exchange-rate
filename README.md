# Bank of Albania Exchange Rates API — bank-of-albania-exchange-rate

[![npm version](https://img.shields.io/npm/v/bank-of-albania-exchange-rate.svg)](https://www.npmjs.com/package/bank-of-albania-exchange-rate)
...
- 🏛️ **Official published rates** — Bank of Albania's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Bank of Albania itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Bank of Albania table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/boa?source=EUR&target=ALL"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/boa').then((x) => x.json());
...
```bash
yarn add bank-of-albania-exchange-rate
```

```bash
pnpm add bank-of-albania-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/bank-of-albania-exchange-rate`](https://www.npmjs.com/package/@allratestoday/bank-of-albania-exchange-rate) — same code, same versions.

## 🏁 Quick start

```js
import { getRate } from 'bank-of-albania-exchange-rate';

const pair = await getRate('EUR', 'ALL', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // the official Bank of Albania rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'bank-of-albania-exchange-rate';
...
```js
import { getRatesForDate } from 'bank-of-albania-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'EUR', target: 'ALL' });
```
...
The Bank of Albania fixes the lek's official exchange rate each business day around midday — roughly 20 currencies plus gold and silver — and that fixing is the legal reference for Albanian customs, tax, and financial reporting.

- Publisher's own page: [Official exchange rate](https://www.bankofalbania.org/Markets/Official_exchange_rate/) · [www.bankofalbania.org](https://www.bankofalbania.org)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Bank of Albania rates page](https://allratestoday.com/central-bank-rates-api/boa/)
...
- Bank of Albania quotes **ALL per 1 unit of foreign currency** (e.g. `base: "USD", quote: "ALL"` means ALL per one US dollar).
...
Loading the official Bank of Albania rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/boa/latest?format=xml&api_key=art_live_..."
```
...
If Bank of Albania does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via ALL from two published rates |
...
| `404` | Pair or date range not covered by Bank of Albania |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'bank-of-albania-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('bank-of-albania-exchange-rate');

getRate('EUR', 'ALL', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Bank of Albania rates page](https://allratestoday.com/central-bank-rates-api/boa/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/bank-of-albania-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/bank-of-albania-exchange-rate)
